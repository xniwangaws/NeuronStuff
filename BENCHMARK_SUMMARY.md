...
* FLUX.2-klein (https://github.com/xniwangaws/NeuronStuff/tree/main/flux2): 9B DiT (klein-base, not guidance-distilled: classic CFG, 2 DiT forwards per step) deployed on trn2.3xlarge with NxDI PR #146 (BF16 TP=4) plus the Qwen3-8B text encoder and VAE decoder on Neuron (2026-09-11; the earlier port left both on the CPU, 35% of the image time). At 1K / 50 steps Neuron achieves 25.9s/image in BF16 (3.0× faster than L4 FP8 at 76.5s, 43% cheaper at $0.016 vs $0.028) and 22.0s/image with FP8 all-Linear W8A8 ($0.0136, SSIM 0.92 vs BF16). The remaining time is 98% DiT at ~45% MFU, the same efficiency FLUX.1-lite reaches; klein needs ~4.5× the FLOPs per image of FLUX.1-lite (1.27× per forward, 100 vs 28 forwards). 2K (184s vs 382.7s, 2.1× faster, 19% cheaper) still uses the CPU text encoder / VAE path. L4 cannot use torch.compile due to BFL custom code graph breaks. 4K is blocked by model spec (max_area=4MP), not hardware — all devices produce gray noise at 4K. Details: flux2/task016_klein_trn2_fp8/.
...
| FLUX.2-klein | 9B | 25.9s / $0.016 (BF16), 22.0s / $0.0136 (FP8 W8A8) | 76.5s / $0.028 | **3.0–3.5× faster, 43–51% cheaper** | 184s / $0.114 (CPU TE/VAE) | 382.7s / $0.141 | **2.1× faster, 19% cheaper** |
