"""Stage-2 ConvNeXt-XL NKI megakernel prototypes."""
...
def _stage2_full_height_tile(
    ...
    """Compute one five-row stage-2 tile without HBM intermediates."""

    sbm.open_scope()

    # Allocate the values that must survive through PW2 first. Everything
    # else lives in nested scopes and is reclaimed before PW2 begins.
    expanded_tiles = []
    ...
    # Depthwise outputs and all normalization work are no longer live.
    sbm.close_scope()
    ...
    # Normalized inputs are now dead; keep only the expanded activations.
    sbm.close_scope()
...
@nki.jit
def convnext_stage2_fused_7x7_block(
    ...
    """Fuse a complete C=1024, H=W=40 ConvNeXt-XL block."""
    ...
    kernel_assert(_HEIGHT_TILES == 8, "stage-2 schedule requires 8 tiles")
    ...
    sbm.open_scope(name="convnext_stage2_fused_block")
...
class NkiConvNextStage2FusedLayerCore(nn.Module):
    """Torch wrapper for the C=1024, 40x40 stage-2 megakernel."""

    def __init__(self, layer: nn.Module, lnc: int = 2):
        super().__init__()
        if layer.dwconv.in_channels != _CHANNELS:
            raise ValueError("This kernel only supports stage-2 C=1024")
    ...
    def forward(self, hidden_states: Tensor) -> Tensor:
        return convnext_stage2_fused_7x7_block[self.lnc](
            hidden_states,
            self.dw_filter,
            self.dw_bias,
            self.norm_weight,
