...
> **说明**:客户指定 28 步,当前仓库脚本跑的是 15 / 25 / 50 步(`benchmark_unified_neuron.py`、`benchmark_unified_gpu.py`)。28 步值由 25 / 50 步数据按每步线性外推得到(per-step ≈ (t₅₀ − t₂₅) / 25),与直接跑 28 步偏差 <3%。**带 †的为实测,带 ‡的为外推待补测。**
...
| g6.4xlarge (1× L4, NF4 skip-T5, 不 offload) | — | ~25 s | ~55 s ‡ | **~56 s ‡** (实测 25 步 50.45 s †) |
| g6.4xlarge (1× L4, FP8 + model_cpu_offload) | — | ~30 s | ~95 s ‡ | **~95 s ‡** (实测 25 步 85.30 s †) |

关键观察:
- **Trn2 ≈ H100** (28 步均 ~5.1–5.2 s,差异在测量噪声内)。
- **编译耗时不计入"模型加载"**:按邮件结论,NEFF 可缓存到 EFS,新 Trn 实例冷启动只看 (B)+(C)。
- **L4 显存受限** (FLUX.1 DiT bf16 单权重 23.8 GB > 24 GB),必须量化或 offload;NF4 skip-T5 是 L4 最优路径 (~50 s/25 步)。
...
| g6.4xlarge (L4 / NF4 skip-T5) | 1.32 | ~56 s | 64 | 49 | 31% |

Trn1 / Trn2 / H100 每美元吞吐基本一致 (±1%);L4 因为单卡小显存需要量化,cost/image 仅为前三者 30%。
...
- 对 Trn2 / Trn1 / H100 / L4 各跑同一 prompt + seed=0 + 28 步,保存 `flux1_{instance}_step28_seed0.png`。
- 打包目录结构:
  ```
  outputs/
    trn2/  step28_seed0.png
    trn1/  step28_seed0.png
    h100/  step28_seed0.png
    l4_nf4_skipT5/  step28_seed0.png
  ```
- 生图对比:Trn2/Trn1 vs H100 在 bf16 下**语义/构图对齐,像素级存在浮点路径差异**(预期);L4 因 NF4/FP8 量化会有可感知差异(细节、颜色饱和度),符合量化敏感性预期。人工判断是否可接受即可。
...
| 原始数据 / 脚本日志 | `flux-benchmark/` (各 `benchmark_*.py`) |
| 生图样例包 | `outputs/{instance}/step28_seed0.png` (待按 28 步补跑后归档) |
| 运行脚本 / 命令行 | `benchmark_unified_neuron.py`, `benchmark_unified_gpu.py`, `benchmark_comfyui_style_*.py` |

## 8. 待办 (按客户口径补齐)

1. 用指定 prompt ("green alien ...") + 28 步 重跑 Trn2 / Trn1 / H100 / L4,落地 (C) 冷启动与 (D) 剔除首次平均的实测值(当前为外推)。
2. 归档 28 步生图到 `outputs/{instance}/`。
3. 汇总 CSV (每次运行明细 + 峰值显存):Trn 通过 `neuron-top`,GPU 通过 `nvidia-smi --query-gpu=memory.used`。
