#!/usr/bin/env python3
"""
FLUX.1-dev benchmark — ComfyUI-style loading with NF4 quantized transformer.

Two modes:
  1. Full:     T5-XXL + CLIP-L → NF4 DiT → VAE  (model_cpu_offload)
  2. Skip T5:  CLIP-L only     → NF4 DiT → VAE  (fits in 24GB without offload)

The NF4 transformer is ~6GB (vs 23.8GB bf16), so skip-T5 mode can fit
everything on a single L4 without any CPU offloading.

Usage:
  python benchmark_comfyui_style_nf4.py                # run both modes
  python benchmark_comfyui_style_nf4.py --mode full
  python benchmark_comfyui_style_nf4.py --mode skip_t5
"""
import argparse
import gc
import subprocess
...
def load_nf4_transformer():
    ...
    print(f"  NF4 transformer loaded in {elapsed:.1f}s | GPU: {get_gpu_info()}")
    return transformer


def build_pipeline_full_nf4():
    """NF4 transformer + T5-XXL + CLIP-L with model_cpu_offload."""
    print("\n=== Loading FULL NF4 pipeline (T5-XXL + CLIP-L) ===")
    ...
    transformer = load_nf4_transformer()
...
def build_pipeline_skip_t5_nf4():
    """NF4 transformer + CLIP-L only — no T5, try to fit on GPU without offload."""
    print("\n=== Loading SKIP-T5 NF4 pipeline (CLIP-L only) ===")
    print(f"GPU before load: {get_gpu_info()}")
    ...
    # NF4 transformer (~6GB) + CLIP-L (~0.5GB) + VAE (~0.2GB) ≈ 7GB
...
def benchmark(pipe, label):
    ...
    if skip_t5:
        ...
        # CLIP-L pooled embedding (768 dim)
        ...
        # T5-XXL replacement: zeros of shape (1, 512, 4096) matching max_sequence_length=512
    ...
    for steps in STEPS_LIST:
        ...
        fname = f"/home/ubuntu/flux_comfy_nf4_{label.replace(' ', '_')}_steps{steps}.png"
        image.save(fname)
...
def print_results(all_results):
    print(f"\n{'='*70}")
    print(f"FLUX.1-dev ComfyUI-Style NF4 Benchmark — Single L4")
...
def main():
    ...
    if args.mode in ("full", "both"):
        pipe = build_pipeline_full_nf4()
        all_results["NF4 full (T5+CLIP-L) offload"] = benchmark(pipe, "full")
        del pipe
        cleanup()

    if args.mode in ("skip_t5", "both"):
        pipe = build_pipeline_skip_t5_nf4()
        all_results["NF4 skip-T5 (CLIP-L only) no-offload"] = benchmark(pipe, "skip_t5")
        del pipe
        cleanup()
