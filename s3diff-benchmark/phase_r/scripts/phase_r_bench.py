"""Phase R multi-image benchmark — shippable eager DeModLoRA pipeline.

Runs 5 test images (cat, bus, bird, butterfly, woman), each with 3 inferences.
Reports per-image cold/warm latency and PSNR vs CPU fp32 reference.

Mirrors Phase E multi-image bench but with DeModLoRA replacement applied.
"""
...
OUT_DIR = "/home/ubuntu/workspace/s3diff_nxdi/data/phase_r_outputs"
...
def build_pipeline():
    ...
    return net_sr, net_de, s3args, device, dtype, len(unet_replaced), len(vae_replaced)
...
def run_one_image(net_sr, net_de, s3args, device, dtype, lq_path, out_path, num_inf=3, seed=123):
    ...
    for i in range(num_inf):
        ...
        if i == 0:
            ...
            saved_path = out_path
    return times, saved_path


def main():
    os.makedirs(OUT_DIR, exist_ok=True)

    print("[build] Phase R pipeline...", flush=True)
    t0 = time.perf_counter()
    net_sr, net_de, s3args, device, dtype, n_unet, n_vae = build_pipeline()
    load_s = time.perf_counter() - t0
    print(f"[build] {load_s:.1f}s  (UNet LoRA replaced: {n_unet}, VAE LoRA replaced: {n_vae})", flush=True)

    results = []
    for name in IMAGES:
        lq_path = f"{LQ_DIR}/{name}_LQ_256.png"
        ref_path = f"{CPU_REF_DIR}/{name}_cpu_fp32.png"
        out_path = f"{OUT_DIR}/{name}_phase_r_bf16.png"
        if not os.path.exists(lq_path):
            print(f"[skip] {name}: LQ not found at {lq_path}", flush=True)
            continue
        if not os.path.exists(ref_path):
            print(f"[skip] {name}: CPU ref not found at {ref_path}", flush=True)
            continue
        ...
        p = psnr_np(ref, new)
        cold = times[0]
        warm_mean = sum(times[1:]) / len(times[1:]) if len(times) > 1 else float("nan")
        print(f"[result] {name}: cold={cold:.2f}s warm_mean={warm_mean:.2f}s PSNR={p:.2f} dB", flush=True)
        results.append({"name": name, "cold": cold, "warm_mean": warm_mean, "psnr": p, "image": saved})

    print("\n" + "="*70)
    print(f"{'image':<12} {'cold':>8} {'warm':>8} {'PSNR':>10}")
    print("-"*70)
    for r in results:
        print(f"{r['name']:<12} {r['cold']:>7.2f}s {r['warm_mean']:>7.2f}s {r['psnr']:>9.2f} dB")
    print("-"*70)
    if results:
        wm = [r["warm_mean"] for r in results]
        ps = [r["psnr"] for r in results]
        print(f"{'mean':<12} {'-':>8} {np.mean(wm):>7.2f}s {np.mean(ps):>9.2f} dB  (std {np.std(ps):.2f})")
        print(f"{'range':<12} {'-':>8} {min(wm):.2f}-{max(wm):.2f}s  {min(ps):.2f}-{max(ps):.2f} dB")
