"""Phase R4c: compile ONLY non-attention sub-components to isolate the compile-bug.

Test matrix:
- scope=non_attn: compile conv_in, conv_out, time_embedding, down_blocks[3] (DownBlock2D),
                  up_blocks[0] (UpBlock2D). Skip all CrossAttn* blocks.
- These are all the UNet submodules that don't contain attention — should avoid
  the unknown compile bug that produced 24 dB output.

Hypothesis: the garbage output is from attention-containing compile units.
Non-attention units should compile cleanly, giving a modest latency win without
quality regression.
"""
...
def parse_args():
    ...
    p.add_argument("--output_image", default="/tmp/phase_r4c_cat.png")
...
def main():
    ...
    print(f"[R4c] replaced UNet LoRA: {len(unet_replaced)}, VAE LoRA: {len(vae_replaced)}", flush=True)
    ...
    # R4c: compile only non-attention submodules
    compile_opts = dict(backend="neuron", dynamic=False, fullgraph=False)

    compiled = []
    # conv_in + conv_out known safe from r4b
    ...
    # time_embedding is a plain MLP
    net_sr.unet.time_embedding.forward = torch.compile(net_sr.unet.time_embedding.forward, **compile_opts)
    compiled.append("time_embedding")
    # Only DownBlock2D/UpBlock2D (no attention)
    import diffusers
    for i, blk in enumerate(net_sr.unet.down_blocks):
        if type(blk).__name__ == "DownBlock2D":
            blk.forward = torch.compile(blk.forward, **compile_opts)
            compiled.append(f"down_blocks[{i}] ({type(blk).__name__})")
    for i, blk in enumerate(net_sr.unet.up_blocks):
        if type(blk).__name__ == "UpBlock2D":
            blk.forward = torch.compile(blk.forward, **compile_opts)
            compiled.append(f"up_blocks[{i}] ({type(blk).__name__})")
    ...
    print()
    print("=== Phase R4c non-attention compile RESULT ===")
