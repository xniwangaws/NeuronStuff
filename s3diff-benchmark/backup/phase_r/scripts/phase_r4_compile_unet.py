"""Phase R4 step 2: try torch.compile(UNet) with DeModLoRA modules in place.

The IRPX901 bug triggers at `...proj_in.lora_A.default|Linear_dot.600`. With
DeModLoRA modules, that IR pattern is gone (no peft adapter ModuleDict, no
intermediate Linear_dot). Worth trying.

Runs once (cold) then 2 warm runs.
"""
...
# Allow CPU fallback on unsupported ops
...
def parse_args():
    ...
    p.add_argument("--output_image", default="/tmp/phase_r4_compile.png")
...
def main():
    ...
    print(f"[R4] replaced UNet LoRA: {len(unet_replaced)}, VAE LoRA: {len(vae_replaced)}", flush=True)
    ...
    # *** R4 step 2: wrap UNet.forward with torch.compile ***
    print("[compile] net_sr.unet.forward = torch.compile(... backend='neuron')...", flush=True)
    orig_unet_fwd = net_sr.unet.forward
    ...
    print()
    print(f"=== Phase R4 torch.compile UNet RESULT ===")
