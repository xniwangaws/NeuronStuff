...
def parse_args():
    ...
    p.add_argument("--lq_size", type=int, default=256)
    p.add_argument("--_dummy", default="x")
    return p.parse_args()
...
def main():
    ...
    im = Image.open(args.lq_image).convert("RGB").resize((args.lq_size, args.lq_size), Image.BICUBIC)
