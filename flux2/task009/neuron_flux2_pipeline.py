...
class NeuronDiT:
    ...
    The underlying NEFF accepts positional inputs in a fixed order and returns
    the joint-sequence noise prediction (B, L_img + L_txt, 128). We expose the
    same keyword-call contract NeuronDiTStub uses so the pipeline scheduler
    loop doesn't change.

    Dtypes (set at trace time, enforced here):
      hidden_states, timestep, encoder_hidden_states : bf16
      guidance                                       : fp32
      img_ids, txt_ids                               : int64
    """
    ...
    def __call__(
        ...
        # Pin input dtypes to the trace contract. Doing this here (not in the
        # caller) keeps the pipeline stub/real DiT swap drop-in.
        ...
        img_ids = img_ids.to(torch.int64)
        txt_ids = txt_ids.to(torch.int64)
...
class NeuronFlux2Pipeline:
    ...
    def from_traced(
        ...
        def text_fn(input_ids, attention_mask=None):
    ...
    def _encode_prompt(self, prompt: str | list[str]):
        ...
        if "attention_mask" in params:
            ...
            out = self.text_encoder(input_ids)
