...
| **Neuron trn2.3xl** | **BF16 TP=4** | **42.90** | 43.28 | **~24 GB** | **10/10** | **$0.02664** | 0.56×(慢 1.78×) | **0.92×**(便宜 8%) |
| L4 g6.4xlarge | **FP8(BFL 官方 ckpt)** | 77.25 | — | 13.12 GB | 10/10 | $0.02839 | 0.31×(慢 3.21×) | 0.98×(便宜 2%) |
...
| **Neuron trn2.3xl** | **BF16 TP=4** | **191.44** | ~40 GB | **10/10** | **$0.1189** | 0.56×(慢 1.79×) | **0.93×**(便宜 7%) |
| L4 g6.4xlarge | **FP8(BFL 官方 ckpt)** | 388.66 | 17.48 GB | 10/10 | $0.1428 | 0.28×(慢 3.63×) | 1.11×(贵 11%) |
...
## 5. DiT 加载 / 冷启动 / 稳态拆分(Neuron 1K / 2K)
...
PSNR 绝对值 ~7-9 dB 是同 prompt 跨 stack diffusion 的正常区间(bf16 数值路径 + solver 步间微小漂移在像素级累积所致),不反映输出可读性。
...
**L4 g6.4xlarge**:DLAMI PyTorch 2.7 / torch 2.8.0+cu128 / **BFL 官方 flux2 repo**(commit `50fe51627778`)+ 自写 FP8 Linear shim(`torch._scaled_mm`, per-tensor E4M3)/ ckpt = `black-forest-labs/FLUX.2-klein-9b-fp8`(guidance-distilled,4-step 原生,此处强制 50-step)/ Qwen3-8B-FP8 文本编码器分阶段加载避免 OOM,AE 转 BF16 以放进 2K decode
...
1. **Neuron klein 1K / 2K × 50 step × 10 seed 全通过**:std=55–95,视觉清晰的猫 + "HELLO WORLD" 纸牌;seed 42 归正,无 variant
...
6. **L4 FP8 可用但无经济优势**:走 BFL 官方 `klein-9b-fp8` + `torch._scaled_mm` per-tensor E4M3 shim,1K 77.25 s / 2K 388.66 s,10/10 pass;$/image = $0.0284 / $0.1428,与 H100 BF16 基本持平(1K 便宜 2% / 2K 贵 11%),但比 Neuron 贵 7–20%。VRAM 占用显著降低(1K 13 GB vs Neuron 24 GB),适合带宽受限的边缘部署;distilled ckpt 在 50-step 下本 prompt 未见 artifact
