...
def _build_sd_loader(weights_dir: str, tp_degree: int):
    ...
    def _load() -> Dict[str, torch.Tensor]:
        ...
        import neuron_flux2_dit_v3 as scaf  # scaffold
...
def _factory_build_model():
    ...
    import torch
    sys.path.insert(0, "/home/ubuntu/flux2_v3")
    import neuron_flux2_dit_v3 as scaf
    from neuronx_distributed_inference.models.config import NeuronConfig
...
def main():
    ...
    ap.add_argument("--name", default="dit_tp8_1024_v3", help="output NEFF filename (no extension)")
    ...
    print(f"[main] saving to {out_path} (this will take a while for 64GB file) ...", flush=True)
    t0_save = time.perf_counter()
    torch.jit.save(traced, out_path)
    save_time = time.perf_counter() - t0_save
    # Force sync before any further HBM activity
    import subprocess
    subprocess.run(["sync"], check=True)
    neff_size = os.path.getsize(out_path)
    print(f"[main] saved to {out_path}  ({neff_size / (1024**2):.1f} MB) in {save_time:.1f}s", flush=True)

    # Skip the dummy forward sanity check: running forward on the just-traced
    # model after save can crash on exit cleanup, and we prefer an intact NEFF
    # over a forward latency print. Smoke test will validate separately.
    print(f"[main] skipping after-save sanity forward; NEFF is written.", flush=True)
    del traced
    import gc as _gc
    _gc.collect()
    print(f"\n=== SUMMARY ===", flush=True)
    print(f"tp_degree       : {args.tp}", flush=True)
    print(f"L_img           : {args.num_patches}", flush=True)
    print(f"L_txt           : {args.text_seq_len}", flush=True)
    print(f"num_layers      : {args.num_layers or hf_cfg.get("num_layers", 8)} double", flush=True)
    print(f"num_single      : {args.num_single_layers or hf_cfg.get("num_single_layers", 48)} single", flush=True)
    print(f"compile_time    : {compile_time:.1f}s", flush=True)
    print(f"save_time       : {save_time:.1f}s", flush=True)
    print(f"neff_path       : {out_path}", flush=True)
    print(f"neff_size       : {neff_size / (1024**2):.1f} MB", flush=True)
    sys.exit(0)
    _DUMMY_SKIP_BELOW = True
    print("UNREACHABLE")
    gc.collect()
    t0 = time.perf_counter()
